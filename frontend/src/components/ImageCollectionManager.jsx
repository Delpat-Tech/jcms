...
const ImageCollectionManager = () => {
  ...
  const loadCollections = useCallback(async () => {
    try {
      ...
      // Set empty collections instead of showing error for now
      setCollections([]);
      // Only show error if it's not a "no collections" scenario
      if (!error.message.includes('retrieve') && !error.message.includes('found')) {
  ...
  const handleMakeCollectionPublic = async (collectionId) => {
    if (!r2Status.configured) {
      alert('Cloudflare R2 is not configured. Cannot make collection public.');
      return;
    }

    try {
      const result = await apiCall(`/api/image-management/collections/${collectionId}/make-public`, {
        method: 'POST'
      });

      if (result.success) {
        alert(`Collection made public! ${result.data.summary.successful} image(s) published.`);
        if (view === 'collections') {
          loadCollections();
        } else {
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center text-sm text-gray-500 mb-3">
                    <span>{collection.stats.totalImages} images</span>
                    <span>{collection.formattedSize}</span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {r2Status.configured && collection.visibility === 'private' && (
                      <Button
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <Button
                      onClick={(e) => {
                        e.stopPropagation();
                        loadCollectionDetail(collection._id);
                      }}
                      variant="secondary"
                      size="sm"
                      className="flex-1"
...
const CollectionDetailView = ({ collection, images, onBack, onMakePublic, r2Status, formatFileSize }) => {
  return (
    <div className="max-w-7xl mx-auto p-6">
      {/* Header */}
      <div className="mb-6">
